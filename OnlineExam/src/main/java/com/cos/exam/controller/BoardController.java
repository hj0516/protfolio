...
public class BoardController {
	...
	@GetMapping("/exam1_perfect")
	public String exam1_perfect(Model model,
			@PageableDefault(size = 100, sort = "id", direction = Sort.Direction.ASC) Pageable pageable) {
		model.addAttribute("exam1", examservice.시험1(pageable));
		return "board/exam1_perfect";
	}
	@GetMapping("/exam2_perfect")
	public String exam2_perfect(Model model,
			@PageableDefault(size = 100, sort = "id", direction = Sort.Direction.ASC) Pageable pageable) {
		model.addAttribute("exam2", examservice.시험2(pageable));
		return "board/exam2_perfect";
	}
	@GetMapping("/mock1_perfect")
	public String mock1_perfect(Model model,
			@PageableDefault(size = 100, sort = "id", direction = Sort.Direction.ASC) Pageable pageable) {
		int[] status = new int[100];
		for (int i = 0; i < status.length; i++) {
			status[i] = i + 1;
		...
		model.addAttribute("exam1", examservice.시험1(pageable));
		model.addAttribute("exam2", examservice.시험2(pageable));
		return "board/mock1_perfect";
	}
	@GetMapping("/mock2_perfect")
	public String mock2_perfect(Model model,
			@PageableDefault(size = 100, sort = "id", direction = Sort.Direction.ASC) Pageable pageable) {
		int[] status = new int[100];
		for (int i = 0; i < status.length; i++) {
			status[i] = i + 1;
